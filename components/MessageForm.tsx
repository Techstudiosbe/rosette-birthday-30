        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-5">
            ...
            <Field name="favourite" label={F.favourite} required>
                <textarea
                    ...
                    rows={3}
                    maxLength={1200}
                    className="resize-y field-paper"
                />
            </Field>

            <Field name="funny" label={F.funny}>
                <textarea id="funny" name="funny" rows={3} maxLength={1200} className="resize-y field-paper" />
            </Field>
            ...
            {status === "error" && (
                <p className="font-body text-terracotta text-sm" role="alert">
                    {error}
                </p>
            )}
...
function Field({
    ...
    return (
        <ScrapbookCard rotate={0} className="px-4 sm:px-5 py-4 sm:py-5">
            <label htmlFor={name} className="block mb-2 font-display text-ink text-base">
...
function SuccessCard() {
    ...
    return (
        <div ref={ref} className="relative mx-auto max-w-md text-center">
            <div className="top-10 left-1/2 absolute -translate-x-1/2 pointer-events-none">
                {emojis.map((e, i) => (
                    <span key={i} className="absolute text-2xl burst">
                        {e}
                    </span>
                ))}
            </div>
            <ScrapbookCard tape="top" tapeTone="washi--coral" className="px-7 py-10">
                ...
                <h2 className="font-display text-ink text-3xl">Thank you!</h2>
                <p className="mx-auto mt-3 max-w-xs font-body text-ink/80">
                    Your words have been sent straight to Rosette. May God bless you for blessing
                    her. 💛
                </p>
                <p className="mt-4 font-hand text-terracotta text-2xl">You are ever so loved.</p>
            </ScrapbookCard>
        </div>
    );
}
